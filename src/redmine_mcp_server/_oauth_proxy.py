...
from ._env import get, get_required, get_required_secret, get_secret
...
def build_oauth_proxy() -> OAuthProxy:
    ...
    upstream_client_id = get("REDMINE_OAUTH_CLIENT_ID") or get_required(
    ...
    jwt_signing_key = get_required_secret(
        ...
        context="OAuth proxy mode",
        guidance="FastMCP uses it to sign proxy tokens and derive encrypted storage.",
    )
    ...
    def redmine_endpoint(path: str) -> AnyHttpUrl:
        """Build a Redmine OAuth endpoint URL from a Redmine issuer URL."""
        return AnyHttpUrl(
            f"{str(AnyHttpUrl(redmine_url)).rstrip('/')}/{path.lstrip('/')}"
        )

    verifier = IntrospectionTokenVerifier(
        introspection_url=str(redmine_endpoint("/oauth/introspect")),
        client_id=introspect_client_id,
        client_secret=introspect_client_secret,
    )

    return OAuthProxy(
        upstream_authorization_endpoint=str(redmine_endpoint("/oauth/authorize")),
        upstream_token_endpoint=str(redmine_endpoint("/oauth/token")),
        upstream_client_id=upstream_client_id,
        upstream_client_secret=upstream_client_secret,
        upstream_revocation_endpoint=str(redmine_endpoint("/oauth/revoke")),
