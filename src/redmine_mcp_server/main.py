...
def build_authenticated_app(mcp_instance, auth_provider):
    ...
    mcp_app = mcp_instance.http_app(path=mcp_path_for_http_app(), stateless_http=True)

    routes = list(auth_provider.get_well_known_routes(mcp_path=metadata_path))
    if metadata_path is not None:
        mount_prefix = "" if mcp_mount_prefix() == "/" else mcp_mount_prefix()
        as_path = f"/.well-known/oauth-authorization-server{mount_prefix}"
        resource_as_path = f"{as_path}{metadata_path}"
        for route in routes:
            if isinstance(route, Route) and route.path == as_path:
                routes.append(
                    Route(
                        resource_as_path,
