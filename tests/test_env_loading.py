...
class TestOAuthIntrospectionEnv:
    ...
    def test_get_returns_plain_env_value(self, monkeypatch):
        monkeypatch.setenv("REDMINE_INTROSPECT_CLIENT_ID", "client-id-x")
        import importlib
        from redmine_mcp_server import _env

        importlib.reload(_env)
        assert _env.get("REDMINE_INTROSPECT_CLIENT_ID") == "client-id-x"

    def test_get_required_raises_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="REDMINE_INTROSPECT_CLIENT_ID"):
            _env.get_required("REDMINE_INTROSPECT_CLIENT_ID")
